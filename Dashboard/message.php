...
$email_verify = $_SESSION['email_utilisateur'];
...
if (isset($_GET['delete'])) {
    ...
    $delete_stmt->execute();
    header("Location: message.php"); // Redirection après suppression
    exit();
}elseif($_GET['delete_all']) {
    $id_msg_to_delete = $_GET['delete'];
    $delete_query = "DELETE * FROM messages WHERE id_msg = ?";
    $delete_stmt = $db->prepare($delete_query);
    $delete_stmt->bind_param("s", $id_msg_to_delete);
...
<body>
    <div class="container mt-5">
        ...
        <a href="?delete_all=<?php echo htmlspecialchars($message['id_msg']); ?>">Supprimer tout les messages</a>
