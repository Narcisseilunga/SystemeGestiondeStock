...
class POS:
    def __init__(self, root_win):
        ...
        # screen Title
        title = Label(self.root, text="Point de Vente", font=("Helvetica", 26, "bold"), bg="#343A40", fg="white", anchor="w", padx=20)
        title.place(x=10, y=0, relwidth=1, height=70)

        # logout button
        logout_btn = Button(self.root, text="Déconnexion", command=self.logout, font=("Helvetica", 11, "bold"), bd=0, bg="#F66B0E", fg="white")
        ...
        product_title = Label(product_frame, text="Section des Produits", font=("Helvetica", 14, "normal"), bg="#2EB086", fg="white")
        ...
        search_label = Label(product_search_frame, text="Recherche de Produit", font=("Helvetica", 14, "normal"), bg="white", fg="#2EB086")
        search_label.place(x=2, y=5)
        pd_name_label = Label(product_search_frame, text="Nom de produit", font=("Helvetica", 13, "normal"), bg="white")
        pd_name_label.place(x=2, y=40)
        pd_name_txt = Entry(product_search_frame, textvariable=self.search_var, font=("Helvetica", 13, "normal"), bg="#EEE6CE")
        pd_name_txt.place(x=125, y=40, width=150, height=22)
        search_btn = Button(product_search_frame, text="Chercher", command=self.search_product, font=("Helvetica", 13, "normal"), bg="#2EB086", fg="white")
        search_btn.place(x=280, y=40, width=110, height=22)
        show_all_btn = Button(product_search_frame, text="Afficher tout", command=self.show_product, font=("Helvetica", 13, "normal"), bg="#313552", fg="white")
        ...
        note_label = Label(product_frame, text="Note: Entrez 0 pour enlever le produit du panier", font=("Helvetica", 12, "normal"), fg="#B22727", bg="white")
        ...
        cust_title = Label(customer_frame, text="Information du client", font=("Helvetica", 14, "normal"), bg="#2EB086", fg="white")
        cust_title.pack(side=TOP, fill=X)

        cust_name_label = Label(customer_frame, text="Nom", font=("Helvetica", 14, "normal"), bg="white")
        cust_name_label.place(x=5, y=35)
        cust_name_txt = Entry(customer_frame, textvariable=self.cust_name_var, font=("Helvetica", 14, "normal"), bg="#EEE6CE")
        cust_name_txt.place(x=80, y=35, width=180)

        cust_contact_label = Label(customer_frame, text="Contact No.", font=("Helvetica", 14, "normal"), bg="white")
        cust_contact_label.place(x=270, y=35)
        cust_contact_txt = Entry(customer_frame, textvariable=self.cust_contact_var, font=("Helvetica", 14, "normal"), bg="#EEE6CE")
        ...
        self.cart_title = Label(cart_frame, text="Panier   | \tTotal des Produits:  0 ", font=("Helvetica", 14, "normal"), bg="#2EB086", fg="white")
        ...
        self.cart_list_table = ttk.Treeview(cart_frame, columns=list_columns_cart, yscrollcommand=scroll_y.set, xscrollcommand=scroll_x.set)
        ...
        p_name_label = Label(cart_widgets_frame, text="Nom de produit", font=("Helvetica", 13, "normal"), bg="white")
        p_name_label.place(x=5, y=5)
        p_name_text = Entry(cart_widgets_frame, textvariable=self.p_name_var, font=("Helvetica", 13, "normal"), state="readonly", bg="#EEE6CE")
        p_name_text.place(x=5, y=35, width=190, height=22)

        p_price_label = Label(cart_widgets_frame, text="Prix par Qte", font=("Helvetica", 13, "normal"), bg="white")
        p_price_label.place(x=230, y=5)
        p_price_text = Entry(cart_widgets_frame, textvariable=self.p_price_var, font=("Helvetica", 13, "normal"), state="readonly", bg="#EEE6CE")
        p_price_text.place(x=230, y=35, width=150, height=22)

        p_qty_label = Label(cart_widgets_frame, text="Quantité", font=("Helvetica", 13, "normal"), bg="white")
        p_qty_label.place(x=400, y=5)
        p_qty_text = Entry(cart_widgets_frame, textvariable=self.p_qty_var, font=("Helvetica", 13, "normal"), bg="#EEE6CE")
        p_qty_text.place(x=400, y=35, width=120, height=22)

        self.p_stock_label = Label(cart_widgets_frame, text="En Stock", font=("Helvetica", 13, "normal"), bg="white")
        self.p_stock_label.place(x=5, y=70)

        add_cart_btn = Button(cart_widgets_frame, text="Ajouter au Panier", command=self.add_cart, font=("Helvetica", 12, "normal"), bg="#0AA1DD", fg="white")
        add_cart_btn.place(x=180, y=70, width=150, height=30)
        clear_cart_btn = Button(cart_widgets_frame, text="Effacer", command=self.clear_cart, font=("Helvetica", 12, "normal"), bg="#313552", fg="white")
        ...
        bill_title = Label(bill_frame, text="Facture du Client", font=("Helvetica", 14, "normal"), bg="#2EB086", fg="white")
    ...
    def generate_bill(self):
        if self.cust_name_var.get() == "" or self.cust_contact_var.get() == "":
            ...
            ecrire_dans_rapport(date_str, "Enregistrement de vente de facture", self.bill_area_text.get())

    def bill_top(self):
        self.invoice_n = int(time.strftime("%H%M%S")) + int(time.strftime("%d%m%Y"))
        print(self.invoice_n)
        bill_top_temp = f'''
\tChalsy's Compagny-Inventaire
\tTelephone No. 971001691 , Lubumbashi
{str("="*45)}
...
if len(sys.argv) >= 2:
    ...
    messagebox.showwarning(fenetre, "Accès impossible \n Veuillez vous connecter en tant qu'administrateurs pour effectuer cette opération pour accéder au poste de vente ")
    fenetre.destroy()
    os.system("python login.py")
