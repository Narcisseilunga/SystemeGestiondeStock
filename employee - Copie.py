...
class Employee:
    def __init__(self, root_win):
        ...
        info_label = Label(content_frame, text="Statut", font=("Helvetica", 14), bg="white")
        info_label.grid(row=4, column=1, padx=10, pady=5, sticky=W)
        info_txt = Entry(content_frame, textvariable=self.info_var, font=("Helvetica", 14), bg="#EEE6CE", bd=1)
        info_txt.grid(row=4, column=2, padx=10, pady=5, sticky=W)

        statut_label = Label(content_frame, text="Statut", font=("Helvetica", 14), bg="white")
        statut_label.grid(row=4, column=3, padx=10, pady=5, sticky=W)
        ...
        statut_opt.grid(row=4, column=4, padx=5, pady=5, sticky=W)
        image_select = Button(content_frame, text="Ajouter une image", command=self.select_image, font=("Arial", 12), bg="#FF5722", fg="white", bd=3, cursor="hand2")
        image_select.grid(row=4, column=5, padx=10, pady=10, sticky="ew")
        ...
        list_columns = ("id", "nom", "email", "sexe", "contact", "date.naiss", "date.adh", "password", "type", "adresse", "salaire", "statut", "Fonction")
        ...
        self.emp_list_table.heading("Fonction", text="Fonction")
        ...
        self.emp_list_table.column("Fonction", width=100)
        ...
        try:
            if self.emp_id_var.get() == "":
                ...
                if row is not None:
                    ...
                    cur.execute("INSERT INTO employee (id, name, email, gender, contact, dob, doj, password, type, address, salary, statut_emp, image_emp, info_emp) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)", values_to_insert)
                    con.commit()
        ...
        try:
            if self.emp_id_var.get() == "":
                ...
                if row is None:
                    ...
                    cur.execute("UPDATE employee set name=?, email=?, gender=?, contact=?, dob=?, doj=?, password=?, type=?, address=?, salary=?, statut_emp, info_emp where id=?", values_to_insert)
                    con.commit()
